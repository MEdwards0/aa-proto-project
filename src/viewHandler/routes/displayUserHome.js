    ...
    return displayUserHome = async (req, res) => {
        ...
        try {
            if (user.token != undefined && user.activeAccount && user.loggedIn) {
                ...
                result.status ? log(req).info({ "session_id": session.id, "message": 'Rendering user_home' }) : log(req).info({ "session_id": session.id, "message": 'Token invalid. Redirecting to /log-out.' });
            ...
            log(req).debug({ "session_id": session.id, "error": `displayUserHome.js error: ${error}` });
            // console.log(error);
            req.session.destroy(); // destroy the current session.
            log(req).info({ "session_id": session.id, "message": "Caught error. Redirecting to Log in screen" });
            res.redirect('/');
        };
    };
};
