...
const wrapper = controller => {
    return manageUsers = async (req, res) => {
        ...
        log(req).info({ "session_id": session.id, "message": 'go to /manage-users' });
        ...
        if (user.token != undefined && user.activeAccount && user.loggedIn && user.userLevel == 'admin') {
            try {
                ...
                if (result.error) {
                    ...
                    log(req).info({ "session_id": session.id, "message": 'Rendering manage_users' });
                ...
                log(req).debug({ "session_id": session.id, "message": `manageUsers.js error: ${error}` });
                req.session.destroy();
