const wrapper = (controller, log) => {
    return validateNinoForm = async (req, res) => {
        ...
        log(req).info({ "session_id": session.id, "message": 'go to /validate-nino.' });
            ...
            if (user.token != undefined && user.activeAccount && user.loggedIn) {
                try {
                    ...
                    result.status ? log(req).info({ "session_id": session.id, "message": 'Rendering validate_nino' }) : log(req).info({ "session_id": session.id, "message": 'Token invalid. Redirecting to /log-out' });

                    result.status ? res.render('validate_nino', { page: page }) : res.redirect('/log-out');

                } catch (error) {
                    log(req).debug({ "session_id": session.id, "error": `validateNinoForm.js error: ${error}`});
                    req.session.destroy(); // destroy the current session.
                    log(req).info({ "session_id": session.id, "message": "Caught error. Redirecting to /" });
                    res.redirect('/');
                };
            } else {
                log(req).debug({"session_id": session.id, "permissions": {
